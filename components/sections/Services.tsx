...
const ICONS: Record<string, React.ReactNode> = {
  ...
  genetics: <svg viewBox="0 0 56 56" fill="none"><rect x="10" y="32" width="6" height="14" rx="1.5" fill="rgba(74,226,74,.8)"/><rect x="20" y="24" width="6" height="22" rx="1.5" fill="rgba(91,143,255,.85)"/><rect x="30" y="28" width="6" height="18" rx="1.5" fill="rgba(226,74,74,.75)"/><rect x="40" y="20" width="6" height="26" rx="1.5" fill="rgba(255,200,50,.75)"/><text x="8" y="54" fill="rgba(74,226,74,.8)" fontFamily="Archivo Wide" fontSize="7">A</text><text x="22" y="54" fill="rgba(91,143,255,.9)" fontFamily="Archivo Wide" fontSize="7">C</text><text x="32" y="54" fill="rgba(226,74,74,.8)" fontFamily="Archivo Wide" fontSize="7">G</text><text x="42" y="54" fill="rgba(255,200,50,.8)" fontFamily="Archivo Wide" fontSize="7">T</text></svg>,
};
...
export default function Services() {
  ...
  return (
    <section className="services" id="services">
      ...
      <div className="svc-shell reveal-up" style={{ '--delay': '0.15s' } as React.CSSProperties}>
        ...
        <div className="svc-tabs" role="tablist">
          {SERVICE_TABS.map(t => (
            <button
              ...
              className={`svc-tab${active === t.id ? ' svc-tab--active' : ''}`}
              onClick={() => setActive(t.id)}
            >
              <span className="svc-tab-num">{t.num}</span>
              <span className="svc-tab-label">{t.label}</span>
            </button>
          ))}
        </div>
        ...
        <div className="svc-panel" key={active}>
          <div className="svc-panel-left">
            <div className="svc-panel-icon">{ICONS[active]}</div>
            <h3 className="svc-panel-title">{tab.title.replace('\n', ' ')}</h3>
            <p className="svc-panel-desc">{tab.desc}</p>
            <div className="svc-panel-turnaround">
            ...
            <p className="svc-panel-note">{detail.note}</p>
            <a href="#booking" className="svc-item-cta" style={{ marginTop: 'auto' }}>
              Rezervo Testin
          ...
          <div className="svc-panel-right">
            <div className="svc-panel-stat">
              <span className="svc-stat-val">{detail.stat.val}</span>
              <span className="svc-stat-lbl">{detail.stat.label}</span>
            </div>
            <div className="svc-panel-tests">
              {detail.tests.map(t => (
                <span key={t} className="svc-item-test">{t}</span>
              ))}
            </div>
          </div>
        </div>
